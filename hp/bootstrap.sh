...
#------------------------------------------------------------------------------
  ...
  Usage: $0 <ProductBundleName>

    eg: where <ProductBundleName> might be ProLiantSupportPack
        or any directory found in ${METHOD}://${WAYSTATION}${URLPREFIX}/downloads
    ...
    get_architecture () {
      ...
      case $my_value in
        hppa|parisc|parisc32|parisc64)
          MSG_TYPE="ERROR" && MSG="currently no downloads for this architecture : $my_value"
...
# auto-generated by
#   ${my_Method}://${my_Waystation}${my_URLPrefix}/downloads/$0 ${Bundle}
...
deb ${my_Method}://${my_Waystation}${my_URLPrefix}/downloads/${Bundle} ${Release}/${my_Revision} non-free
...
baseurl=${my_Method}://${my_Waystation}${my_URLPrefix}/downloads/${Bundle}/${Distribution}/${my_rel}/${my_arch}/${my_Revision}
enabled=1
gpgcheck=0
#gpgcheck=1
#gpgkey=${my_Method}://${my_Waystation}${my_URLPrefix}/downloads/${Bundle}/GPG-KEY-${Bundle}

#[HP-${Bundle}-packages]
#name=HP Software Delivery Repository Repository for ${Bundle} Packages
#baseurl=${my_Method}://${my_Waystation}${my_URLPrefix}/downloads/${Bundle}/${Distribution}/${my_rel}/packages/${my_arch}
#enabled=0
#gpgcheck=0
#gpgcheck=1
#gpgkey=${my_Method}://${my_Waystation}${my_URLPrefix}/downloads/${Bundle}/GPG-KEY-${Bundle}
...
enabled=1
autorefresh=1
baseurl=${my_Method}://${my_Waystation}${my_URLPrefix}/downloads/${Bundle}/${Distribution}/${Release}/${Architecture}/${my_Revision}
path=/
type=YUM
priority=100
keeppackages=0
...
#enabled=0
#baseurl=${my_Method}://${my_Waystation}${my_URLPrefix}/downloads/${Bundle}/${Distribution}/${Release}/packages/${Architecture}
#path=/
#type=YUM
#priority=100
#keeppackages=0
...
#------------------------------------------------------------------------------
  ...
  # legal/licensing requirements
    ...
    [ -z "$PREVIEW" ] && [ -x $WGET ] && {
      ...
      # connectivity check
      if $WGET --quiet --server-response --spider --timeout=4 --tries=2 ${my_Method}://${my_Waystation}${my_URLPrefix}/downloads/${Bundle} >/dev/null 2>&1 ; then
        TMPFILE=$(mktemp /tmp/EULA.XXXXXXXX) && {

          # EULA, if applicable
          if $WGET --quiet ${my_Method}://${my_Waystation}${my_URLPrefix}/downloads/${Bundle}/${EULA} --output-document=$TMPFILE 2>/dev/null; then
            [ -s $TMPFILE ] && {
        ...
        echo "$MSG_TYPE : $MSG"
        MSG_TYPE="warn" && MSG="${my_Method}://${my_Waystation}${my_URLPrefix}/downloads/${Bundle}"
        echo "$MSG_TYPE : $MSG"
      fi
    }
  ...
  # configure that repository
    ...
    case $Distribution in
      ...
      Debian|Ubuntu)
        ...
        if $WGET --quiet --spider --timeout=4 --tries=2 ${my_Method}://${my_Waystation}${my_URLPrefix}/downloads/${Bundle}/dists/${Release}/${my_Revision} >/dev/null 2>&1 ; then
		...
		answer="yes"
          ...
          echo "$MSG_TYPE : $MSG"
          MSG_TYPE="warn" && MSG="${my_Method}://${my_Waystation}${my_URLPrefix}/downloads/${Bundle}/dists/${Release}/${my_Revision}"
          echo "$MSG_TYPE : $MSG"
      ...
      *)
        ...
        if $WGET --quiet --spider --timeout=4 --tries=2 ${my_Method}://${my_Waystation}${my_URLPrefix}/downloads/${Bundle}/${Distribution}/${Release}/${Architecture}/${my_Revision} >/dev/null 2>&1 ; then
		...
		#read answer
          ...
          MSG_TYPE="warn" && MSG="Unable to find relevant deliverables at URL : "
          echo "$MSG_TYPE : $MSG"
          MSG_TYPE="warn" && MSG="${my_Method}://${my_Waystation}${my_URLPrefix}/downloads/${Bundle}/${Distribution}/${Release}/${Architecture}/${my_Revision}"
          echo "$MSG_TYPE : $MSG"
          MSG_TYPE="note" && MSG="No repository configurations added."
          echo "$MSG_TYPE : $MSG"
        fi
